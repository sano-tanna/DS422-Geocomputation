...
```{r}
unzip(here("data/FirespotArea_LANDSAT_USA_contiguous_and_Hawaii_7d.kmz", exdir = "data/fire_kml"))

wildfire_area_data <- st_read(here("data/fire_kml/LANDSAT_USA_contiguous_and_Hawaii_7d_1757637915448.kml"),
...
fires_df <- read.csv(here("data/LANDSAT_USA_contiguous_and_Hawaii_7d.csv"))
...
leaflet(fires_sf) |>
  ...
  addCircleMarkers(
    radius = 4,
    color = ~ifelse(daynight == "D", "orange", "blue"),
    fillOpacity = 0.7,
    popup = ~paste("Date:", acq_date, "<br>Confidence:", confidence)
  ) |>
  addLegend(

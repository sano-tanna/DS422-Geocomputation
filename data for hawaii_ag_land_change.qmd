...
df <- bind_rows(
  ag15 %>%
    ...
    mutate(area_ha = as.numeric(st_area(.)) / 10000, year = "2015"),
  ag20 %>%
    rename(crop = crops_2020) %>%
    mutate(area_ha = as.numeric(st_area(.)) / 10000, year = "2020")
) %>%
  st_drop_geometry() %>%
  group_by(year, crop) %>%
...
ggplot(df, aes(x = reorder(crop, area), y = area, fill = year)) +
  ...
  coord_flip() +
  labs(
    title = "Agricultural Area by Crop (2015 vs 2020)",
...
tm_layout(title = "Agriculture Gain (green) & Loss (red), 2015→2020", legend.show = FALSE)
